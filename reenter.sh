...
export TF_VAR_argocd_webhook_secret="$(kubectl get secret -n argocd argo-workflows-sso -o json | jq -r '.data | map_values(@base64d) | ."client-id"')"
...
export TF_VAR_github_webhook_secret="$(kubectl get secret -n argocd argo-workflows-sso -o json | jq -r '.data | map_values(@base64d) | ."client-id"')"
